...
# Convert to numeric
numeric_cols <- c("RM", "AM", "n_children", names(table8)[4:41])
...
# Key covariates from the paper (15 total across 5 categories)
...
covariates_to_test <- list(
  Segregation = c(
    "Racial.Segregation",
    "Income.Segregation",
    "Segregation.of.Poverty...p25.",   # "(<p25)" → "...p25."
    "Segregation.of.Affluence...p75."  # "(>p75)" → "...p75."
  ),
  Inequality = c(
    "Gini",
    "Top.1..Income.Share",              # "1%" → "1.."
    "Gini.Bottom.99.",                  # "99%" → "99."
    "Frac..Between.p25.and.p75"
  ),
  Education = c(
    "School.Expenditure.per.Student",
    "Test.Score.Percentile..Income.Adjusted",  # Missing ) in CSV!
...
cat(sprintf("✓ Testing %d covariates across 5 categories\n\n", length(all_covariates)))
...
phase2_results_clean <- phase2_results %>%
  mutate(covariate = case_when(
    ...
    covariate == "Frac..Between.p25.and.p75" ~ "Fraction Between p25 and p75",
