---
title: "Classical vs Permutation Testing for Relative Mobility Analysis"
author: "Max Miller"
date: "`r Sys.Date()`"
output:
  pdf_document:
...
This analysis tests both **CORRELATION** and **REGRESSION** to demonstrate equivalence between classical and permutation testing approaches for relative mobility analysis.

# Variable Definitions

## Console Output Variables

The analysis prints results in the following format:

```
[OK] Racial.Segregation  b=0.406 p_c=0.000 p_pc=0.000 p_pr=0.000*
```

| Symbol | Name | Description |
|--------|------|-------------|
| `[OK]` | Agreement Status | All methods agree on significance. `[X]` indicates disagreement. |
| `[X]` | Disagreement Status | Classical and permutation methods disagree on significance. |
| `b` | Beta Coefficient | Standardized regression coefficient (equals Pearson r when both X and Y are standardized). |
| `p_c` | Classical p-value | p-value from OLS regression (Method 2). |
| `p_pc` | Permutation Correlation p-value | p-value from permutation correlation test using perk/PermCor (Method 3). |
| `p_pr` | Permutation Regression p-value | p-value from permutation regression test based on DiCiccio & Romano Theorem 3.2 (Method 4). |
| `*` | Significance Indicator | Asterisk indicates statistical significance at α = 0.05. |

## CSV Output Variables (mobility_test_results.csv)

| Column | Description |
|--------|-------------|
| `category` | Thematic grouping: Segregation, Inequality, Education, Family, or Social. |
| `covariate` | Predictor variable name (e.g., "Racial Segregation"). |
| `n_obs` | Number of observations (CZs) after removing missing values. |
...
| `reg_coefficient` | OLS regression slope β from `lm()` (Method 2). Equals `cor_coefficient` when standardized. |
| `reg_se` | Standard error of the regression coefficient. |
| `reg_t_stat` | t-statistic from regression. Equals `cor_t_stat` when standardized. |
| `reg_pval` | p-value from regression. Equals `cor_pval` when standardized. |
| `methods_agree` | TRUE if correlation and regression results match (diff < 0.001). |
| `perm_cor_pval` | p-value from permutation correlation test (Method 3). |
| `perm_reg_pval` | p-value from permutation regression test (Method 4). |
...
| `all_agree` | TRUE if all methods agree on significance (all significant or all not). |

## Methods Summary

| Method | Function | Test Type | Source |
|--------|----------|-----------|--------|
| Method 1 | `cor.test()` | Classical Pearson correlation | Base R |
| Method 2 | `lm()` | Classical OLS regression | Base R |
| Method 3 | `perk_test()` or `perm_test()` | Permutation correlation | perk/PermCor packages |
| Method 4 | `perm_test_regression()` | Permutation regression | Alex's implementation of DiCiccio & Romano (2017) Theorem 3.2 |
...
# DEBUG: Show ALL actual column names after R processing
# cat(sprintf("Table 8 has %d columns:\n\n", length(names(table8))))
# for(i in 1:length(names(table8))) {
#  cat(sprintf("%2d: %s\n", i, names(table8)[i]))
# }
...
cat(sprintf(
  "Sample: %d CZs with n>=250 (out of %d total CZs)\n\n",
  nrow(df), nrow(data)
))
```
...
```{r covariates}
# FULL TABLE VIII: All 35 covariates from Online Appendix Table VIII
...
cat(sprintf(
  " Testing %d covariates across %d categories\n\n",
  length(all_covariates), length(covariates_to_test)
))
```

# Phase 1: Replicate Classical Results (OLS)

This phase replicates the classical OLS results from Online Appendix Table VIII. We test both CORRELATION and REGRESSION (they should give identical results).
...
for (category_name in names(covariates_to_test)) {
  ...
  for (covariate_name in covariates_to_test[[category_name]]) {
    ...
    # Print result
    agreement_icon <- ifelse(methods_agree, "[OK]", "[X]")
    sig_icon <- ifelse(reg_p < 0.05, "*", " ")

    cat(sprintf(
      "  %s %-30s n=%3d b=%5.3f p=%5.3f%s\n",
      agreement_icon,
...
cat(sprintf(
  "  Significant at alpha=0.05: %d (%.1f%%)\n",
  sum(phase1_results$reg_pval < 0.05),
  100 * mean(phase1_results$reg_pval < 0.05)
))
cat(sprintf(
  "  Correlation approx Regression: %d/%d (%.1f%% agreement)\n",
...
This phase adds permutation tests and compares them to classical results. We run both CORRELATION and REGRESSION permutation tests.
...
for (i in 1:nrow(phase2_results)) {
  ...
  # Print result
  agreement_icon <- ifelse(all_agree, "[OK]", "[X]")
  sig_icon <- ifelse(classical_sig, "*", " ")

  if (!is.na(perm_cor_p)) {
    cat(sprintf(
      "  %s %-25s b=%5.3f p_c=%5.3f p_pc=%5.3f p_pr=%5.3f%s\n",
      agreement_icon,
    ...
    cat(sprintf(
      "  %s %-25s b=%5.3f p_c=%5.3f p_pr=%5.3f%s\n",
      agreement_icon,
      substr(covariate_name, 1, 25),
      phase2_results$reg_coefficient[i],
      phase2_results$reg_pval[i],
...
```{r final-summary}
cat("1. PHASE 1: Classical Results\n")
cat(sprintf("   - Total tests: %d\n", nrow(phase2_results)))
cat(sprintf(
  "   - Significant (classical): %d (%.1f%%)\n",
  sum(phase2_results$classical_sig),
  100 * mean(phase2_results$classical_sig)
))
cat(sprintf(
  "   - Correlation approx Regression: %.1f%% agreement\n",
  100 * mean(phase2_results$methods_agree)
))

cat("2. PHASE 2: Permutation Results\n")
cat(sprintf(
  "   - Significant (perm regression): %d (%.1f%%)\n",
...
```{r visualizations, fig.width=10, fig.height=8, message=FALSE}
...
p2 <- ggplot(phase2_results_plot, aes(x = reg_coefficient, y = covariate_short)) +
  ...
  geom_errorbar(
    aes(
      xmin = reg_coefficient - 1.96 * reg_se,
...
```{r conclusion}
cat(sprintf("  - Tested %d covariates across %d categories\n", nrow(phase2_results), length(covariates_to_test)))
cat(sprintf(
  "  - Classical significance: %d/%d (%.1f%%)\n",
  sum(phase2_results$classical_sig),
  nrow(phase2_results),
  100 * mean(phase2_results$classical_sig)
))
cat(sprintf(
  "  - Permutation significance: %d/%d (%.1f%%)\n",
  sum(phase2_results$perm_reg_sig),
  nrow(phase2_results),
  100 * mean(phase2_results$perm_reg_sig)
))
